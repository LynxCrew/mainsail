<template>
    <v-menu :offset-y="true" :close-on-content-click="false" :title="$t('Panels.TemperaturePanel.SetupTemperatures')">
        ...
        <v-list>
            ...
            <v-list-item class="minHeight36">
                <v-checkbox
                    v-model="hideHeaterProfiles"
                    class="mt-0"
                    hide-details
                    :label="$t('Panels.TemperaturePanel.HideHeaterProfiles')" />
            </v-list-item>
            <v-list-item class="minHeight36">
                <v-checkbox
...
export default class TemperaturePanelSettings extends Mixins(BaseMixin) {
    ...
    set hideHeaterProfiles(newVal: boolean) {
        this.$store.dispatch('gui/saveSetting', { name: 'view.tempchart.hideHeaterProfiles', value: newVal })
    }
}
</script>
