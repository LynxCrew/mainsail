...
export default class EndstopPanel extends Mixins(BaseMixin) {
    ...
    get items() {
        ...
        if ('beacon' in this.$store.state.printer && 'last_query' in this.$store.state.printer.beacon) {
            const value = this.$store.state.printer.beacon.last_query ? 'TRIGGERED' : 'open'

            output.push({
                type: 'probe',
                name: this.$store.state.printer.beacon.name ?? 'probe',
                value,
            })
        }
    ...
    get existsQueryProbe() {
        ...
        return 'probe' in this.$store.state.printer || 'beacon' in this.$store.state.printer
    }
